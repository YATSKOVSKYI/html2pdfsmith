...
function fileNameFor(slug, slot, sourcePath) {
  const ext = extname(sourcePath).toLowerCase() || ".ttf";
  return `${slug}-${slot}${ext}`;
}
function toManifestPath(fromDir, filePath) {
...
function parseInstallArgs(args) {
  ...
  const dedupedFamilies = [...new Map(requestedFamilies.map((family) => [family.trim().toLowerCase(), family.trim()])).values()].filter(Boolean);
...
async function copyVariant(outDir, familySlug, slot, sourcePath) {
...
function manifestFace(outDir, face) {
  const out = {
    family: face.family,
...
function cssForFace(outDir, face) {
  const entries = [
...
async function writeInstallReadme(outDir, manifestPath, cssPath) {
  const lines = [
...
async function installFonts(args) {
